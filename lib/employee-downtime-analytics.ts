...
import { formatDurationMinutes } from "@/lib/machine-activity-analytics"
...
function dedupeIncidents(incidents: EmployeeDowntimeIncident[]): EmployeeDowntimeIncident[] {
  const byBucket = new Map<string, EmployeeDowntimeIncident>()
  for (const inc of incidents) {
    const ts = new Date(inc.occurredAt).getTime()
    const bucket = `${inc.machineId ?? "—"}|${Math.floor(ts / 300_000)}`
    const existing = byBucket.get(bucket)
    if (!existing || inc.stage > existing.stage) {
      byBucket.set(bucket, inc)
    }
  }
  return [...byBucket.values()].sort((a, b) => b.occurredAt.localeCompare(a.occurredAt))
}
...
export function buildEmployeeDowntimeAnalytics(input: {
  ...
  const pushIncident = (opts: {
    ...
    const durationMinutes = opts.stage === 2 ? stage2 : stage1
    const sourceKey = downtimeSourceKey(opts.machineId, opts.occurredAt, opts.stage)
    ...
    rawIncidents.push({
      ...
      stage: opts.stage,
      durationMinutes,
      stageLabel: `Tipo ${opts.stage} (${durationMinutes} min)`,
      reason: opts.reason,
  ...
  for (const a of input.alerts) {
    if (!isParoAlertTitle(a.title)) continue
